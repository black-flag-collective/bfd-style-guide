...
Agora is a shared workspace, not a generic dashboard. Home opens directly on projects and delivery, without a page banner or eyebrow. A project card shows the actual project or client logo, name, and direct opening action. Show purpose where it helps someone choose; Home may use compact identity tiles. Project and client marks sit on a solid light surface at full opacity, with a neutral fallback when no logo exists. Activity cards keep vendor marks recognizable without hover, expose a clear Open action, and show enough conversation context to decide whether to read further. Search and secondary controls open when needed; source and date remain quick to reach.
...
Optional detail uses disclosure. Keep the selected secondary view visible and preserve its URL when opening a drawer. Do not remove capabilities to simplify a screen. On refresh, keep the current records and user input visible; reserve skeletons for the first load. Storybook should cover ready, loading, refreshing, empty, failed, long-content, keyboard, and narrow-screen states. Guide section headings help readers browse this document; they are not a pattern for app page banners.
