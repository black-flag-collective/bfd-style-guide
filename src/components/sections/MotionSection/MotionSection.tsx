        ...
        <div className="max-w-6xl mx-auto">
              <SectionHeader
                ...
                actions={
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 btn-active rounded-lg text-sm font-medium hover:opacity-90 transition-opacity w-fit flex-shrink-0"
                  >
                    <Play size={14} strokeWidth={2} /> Replay
                  </button>
                }
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" key={replayKey}>
                ...
                <div className="bg-bf-surface rounded-lg p-6">
                  <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Fade In Up</h3>
                  <motion.div
                    initial={{ opacity: 0, y: 30 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
                    className="h-16 btn-active rounded-lg flex items-center justify-center"
                  >
                    <span className="text-sm font-medium">0.8s / cubic-bezier(0.4, 0, 0.2, 1)</span>
                  </motion.div>
                </div>

                {/* Stagger Children */}
                <div className="bg-bf-surface rounded-lg p-6">
                  <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Stagger Children</h3>
                ...
                {/* Hover Scale */}
                <div className="bg-bf-surface rounded-lg p-6">
                  <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Hover Scale</h3>
                  <motion.div
                    whileHover={{ scale: 1.05 }}
                    transition={{ duration: 0.3, ease: "easeOut" }}
                    className="h-16 btn-active rounded-lg flex items-center justify-center cursor-pointer"
                  >
                    <span className="text-sm font-medium">Hover me &mdash; scale 1.05</span>
                  </motion.div>
                </div>

                {/* Comic Shadow Hover */}
                <div className="bg-bf-surface rounded-lg p-6">
                  <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Comic Shadow</h3>
                  <motion.div
                    whileHover={{ x: -2, y: -2 }}
                    transition={{ duration: 0.3, ease: "easeOut" }}
              ...
              <div className="bg-bf-surface rounded-lg p-4 mb-6 overflow-hidden">
                <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Marquee &mdash; 60s linear infinite</h3>
              ...
              <div>
                <h3 className="text-sm font-medium text-bf-text mb-4 uppercase tracking-wider">Timing Reference</h3>
                <div className="space-y-1">
                  {timingSpecs.map((spec) => (
                    <div key={spec.name} className="hidden sm:grid grid-cols-[8rem_1fr_1fr] gap-4 items-center py-1.5 border-b border-bf-border/30 last:border-0">
                  ...
                  {timingSpecs.map((spec) => (
                    <div key={`m-${spec.name}`} className="sm:hidden py-2 border-b border-bf-border/30 last:border-0">
                      <div className="flex items-baseline justify-between gap-2 mb-0.5">
                        <span className="text-sm font-medium text-bf-text">{spec.name}</span>
                        <span className="text-[10px] text-bf-muted">{spec.usage}</span>
                      </div>
                      <span className="text-xs font-mono text-bf-muted">{spec.value}</span>
                    </div>
                  ))}
